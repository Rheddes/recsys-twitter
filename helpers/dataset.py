...
class MyIterableDataset(IterableDataset):
    ...
    def __init__(self, file_path):
        self.max_token_len = pd.read_csv(file_path, header=None, sep="\x01")[0].apply(lambda x: x.count("\t") + 1).max()
    ...
    def parse_file(self, file_path):
        with open(file_path, encoding="utf-8") as fileobject:
            for line in fileobject:
                features = line.strip().split("\x01")
                tokens = np.fromstring(features[0], sep="\t", dtype=int)
                yield [np.concatenate((tokens, np.zeros(self.max_token_len - len(tokens), dtype=int)))]
